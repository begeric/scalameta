...
class TransformerMacros(override val c: Context) extends TraverserMacros(c) {
  ...
  def makeTransformStat[T : c.WeakTypeTag](field: Field, methodName: TermName): Transformed = {
    ...
    field.tpe match {
      ...
      case t if t <:< weakTypeOf[Option[T]]   =>
        Transformed(field.name, newName, Some(q"val $newName = mapConserve(${field.name}, $methodName).asInstanceOf[$t]"))  //TODO change to a mapConserve like stuff
   ...
   * */
  override def buildCase[T : c.WeakTypeTag](leaf: Leaf, methodName: TermName): Option[c.Tree] = {
